...
using MultiTool.DataStructures.Info;
using MultiTool.Handlers.FileHandlers;
using MultiTool.DataStructures.Constants;

using static LevelModel.Models.Level;

namespace MultiTool.Menu.Options.ExistingLevel.Options.ModifyLevel.Options
{
    internal class RemoveArtOption : BaseMenuOption
    {

        private BuildHandler _builder;
        private RemoveArtInfo _info;
        private bool _quit;
        internal RemoveArtOption() {
            _builder = new BuildHandler();
            _info    = new RemoveArtInfo();

            GetRequiredInfo();

            if (IsInputValid)
                _builder.RemoveArt(_info);
            else if(!_quit)
                WriteLine(Environment.NewLine + "\tError: Invalid input!", ErrorColor);
        }

        private void ShowArtTOptions()
        {
            WriteLine("Art Types:");
